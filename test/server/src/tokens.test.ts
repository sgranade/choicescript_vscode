...
import {
    ...
    ExpressionTokenType,
    ExpressionEvalType,
} from "../../../server/src/common/tokens";

function createDocument(
    text: string,
    uri: string = "file:///scene.txt",
    ...
    describe("Expressions", () => {
        describe("Token Creation", () => {
            it("should tokenize floating point numbers", () => {
                ...
                expect(expression.tokens[0].type).to.equal(
            ...
            it("should flag functions with no arguments", () => {
                ...
                expect(expression.parseErrors[0].message).to.include(
        ...
        describe("Validation", () => {
            describe("Basic", () => {
                it("should note an empty expression", () => {
                    ...
                    expect(expression.evalType).to.equal(
                        ExpressionEvalType.Empty,
                    );
                });
                ...
                it("should flag a bare operator", () => {
                    ...
                    expect(expression.validateErrors[0].message).to.include(
                        "Not a valid value",
                    );
                    expect(
                        expression.validateErrors[0].range.start.line,
                    ).to.equal(2);
                    expect(
                        expression.validateErrors[0].range.end.line,
                    ).to.equal(3);
                    expect(expression.evalType).to.equal(
                        ExpressionEvalType.Error,
                    );
                });
            ...
            describe("Boolean", () => {
                ...
                it("should flag a boolean with a not-boolean second value", () => {
                    ...
                    expect(expression.validateErrors.length).to.equal(1);
                    expect(expression.validateErrors[0].message).to.include(
                        "Must be a boolean value or a variable",
                    );
                    expect(
                        expression.validateErrors[0].range.start.line,
                    ).to.equal(11);
            ...
            describe("String", () => {
                it("should flag an unterminated string", () => {
                    ...
                    expect(expression.validateErrors.length).to.equal(1);
                    expect(expression.validateErrors[0].code).to.equal(
                ...
                it("should flag a string with a math operator", () => {
                    ...
                    expect(expression.validateErrors.length).to.equal(1);
                    expect(expression.validateErrors[0].message).to.include(
                        "Not a string or comparison operator",
                    );
                    expect(
                        expression.validateErrors[0].range.start.line,
                    ).to.equal(7);
            ...
            describe("Variable Setting", () => {
                ...
                it("should flag a boolean operator", () => {
                    ...
                    expect(expression.validateErrors.length).to.equal(1);
                    expect(expression.validateErrors[0].message).to.include(
                        "Must be a numeric operator, value, or variable",
                    );
                    expect(
                        expression.validateErrors[0].range.start.line,
                    ).to.equal(2);
            ...
            describe("Parentheses", () => {
                ...
                it("should be good with parenthesized number expressions", () => {
                    ...
                    expect(subexpression?.evalType).to.equal(
                        ExpressionEvalType.Number,
                    );
                });
                ...
                it("should flag errors in parentheses", () => {
                    ...
                    expect(subexpression?.validateErrors[0].message).to.include(
                        "Must be a number or a variable",
                    );
                    expect(
                        subexpression?.validateErrors[0].range.start.line,
                    ).to.equal(11);
                    expect(
                        subexpression?.validateErrors[0].range.end.line,
                    ).to.equal(16);
                    expect(subexpression?.evalType).to.equal(
                        ExpressionEvalType.Error,
                    );
