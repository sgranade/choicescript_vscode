...
describe("Indexer", () => {
    describe("Symbol Creation Indexing", () => {
        it("should index locations of created global variables", () => {
            ...
            fakeIndex
                .setGlobalVariables(Arg.all())
                .mimicks((uri: string, index: Map<string, Location>) => {
                    received.push(index);
                });
        ...
        it("should index effective locations of local variables created in a subroutine", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setSubroutineLocalVariables(Arg.all())
                .mimicks((uri: string, index: Map<string, Location>) => {
                    received.push(index);
                });
    ...
    describe("Symbol Command Indexing", () => {
        it("should index bare variables in the command", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setVariableReferences(Arg.all())
                .mimicks((uri: string, index: Map<string, Location[]>) => {
                    receivedReferences.push(index);
                });
    ...
    describe("Flow Control Event Indexing", () => {
        it("should capture the flow control command", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setFlowControlEvents(Arg.all())
                .mimicks((uri: string, events: FlowControlEvent[]) => {
                    receivedReferences.push(events);
                });
        ...
        it("should capture the label location", () => {
            ...
            expect(
                receivedReferences[0][0].labelLocation?.range.start.line,
            ).to.equal(7);
            expect(
                receivedReferences[0][0].labelLocation?.range.end.line,
            ).to.equal(12);
        });
        ...
        it("should capture the scene location", () => {
            ...
            expect(
                receivedReferences[0][0].sceneLocation?.range.start.line,
            ).to.eql(13);
            expect(
                receivedReferences[0][0].sceneLocation?.range.end.line,
            ).to.eql(23);
        });
        ...
        it("should capture both scene and label if present", () => {
            let fakeDocument = createDocument(
                "*gosub_scene scene-name other_label",
            );
            let receivedReferences: Array<FlowControlEvent[]> = [];
            let fakeIndex = createIndex();
            fakeIndex
    ...
    describe("Scene Indexing", () => {
        it("should index scenes in startup files", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex.setSceneList(Arg.any()).mimicks((scenes: string[]) => {
                receivedScenes.push(scenes);
            });
        ...
        it("should not add the choicescript_stats scene to the scene list if it's already listed in the startup file", () => {
            ...
            fakeIndex
                .getSceneList()
                .returns(["choicescript_stats", "old_scene"]);
    ...
    describe("Stats Indexing", () => {
        it("should set has-choicescript-stats for stats files", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setHasChoicescriptStats(Arg.any())
                .mimicks((canHas: boolean) => {
                    receivedCalls.push(canHas);
                });
            fakeIndex.getSceneList().returns([]);
    ...
    describe("Achievement Indexing", () => {
        it("should index an achievement", () => {
            ...
            let receivedAchievements: Map<
                string,
                [Location, number, string]
            >[] = [];
            let fakeIndex = createIndex();
            fakeIndex
                .setAchievements(Arg.any())
                .mimicks((index: Map<string, [Location, number, string]>) => {
                    receivedAchievements.push(index);
                });
        ...
        it("should index a reference to an achievement", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setAchievementReferences(Arg.any(), Arg.any())
                .mimicks((uri, index) => {
    ...
    describe("Label Indexing", () => {
        it("should index a label", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setLabels(Arg.any(), Arg.any())
                .mimicks((uri: string, index: LabelIndex) => {
                    receivedLabels.push(index);
                });
        ...
        it("should add a scope to a label on a *return", () => {
            ...
            expect(receivedLabels[0].size).to.equal(1);
            expect(
                receivedLabels[0].get("label_name")?.scope?.start.line,
            ).to.equal(7);
            expect(
                receivedLabels[0].get("label_name")?.scope?.end.line,
            ).to.equal(32);
        });
        ...
        it("should add a scope to the most recent label on a *return", () => {
            ...
            expect(
                receivedLabels[0].get("label_two")?.scope?.start.line,
            ).to.equal(31);
            expect(
                receivedLabels[0].get("label_two")?.scope?.end.line,
            ).to.equal(55);
        });
    });
    ...
    describe("Variable Scoping", () => {
        it("should capture *check_achievements scopes", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setDocumentScopes(Arg.all())
                .mimicks((uri: string, scope: DocumentScopes) => {
                    received.push(scope);
                });
        ...
        it("should have *check_achievements scope run from definition to document end", () => {
            ...
            expect(received[0].achievementVarScopes[0].start).to.eql({
                line: 8,
                character: 0,
            });
            expect(received[0].achievementVarScopes[0].end).to.eql({
                line: 43,
                character: 0,
            });
        });
        ...
        it("should have *params scope run from definition to document end", () => {
            ...
            // These positions are due to the fake document converting an index directly into a line number
            expect(received[0].paramScopes[0].start).to.eql({
                line: 8,
                character: 0,
            });
            expect(received[0].paramScopes[0].end).to.eql({
                line: 31,
                character: 0,
            });
        });
    ...
    describe("Choice Scoping", () => {
        ...
        it("should summarize a *choice block", () => {
            ...
            expect(received[0].choiceScopes[0].summary).to.equal(
                "choice (#One)",
            );
    ...
    describe("Images", () => {
        it("should capture referenced images", () => {
            ...
            expect(Array.from(received[0].keys())).to.eql([
                "cover.jpg",
                "http://faker.com/img.png",
            ]);
    ...
    describe("Script commands", () => {
        it("should capture *script commands", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setScriptUsages(Arg.all())
                .mimicks((sceneUri: string, usages: Location[]) => {
                    received.push(usages);
                });
    ...
    describe("Parse Errors", () => {
        it("should flag attempts to re-create already-created global variables", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setParseErrors(Arg.all())
                .mimicks((uri: string, errors: Diagnostic[]) => {
            ...
            expect(received[0][0].code).to.equal(
        ...
        it("should flag a *return with no label", () => {
            ...
            expect(received.length).to.equal(1);
            expect(received[0].length).to.equal(1);
            expect(received[0][0].message).to.include(
    ...
    describe("Word Counts", () => {
        it("should count words in a document", () => {
            ...
            let fakeIndex = createIndex();
            fakeIndex
                .setWordCount(Arg.all())
                .mimicks((uri: string, count: number) => {
                    received.push(count);
                });
    ...
    describe("Non-Indexed Scenes", () => {
        it("should return scenes that haven't yet been indexed", () => {
            ...
            fakeIndex
                .getAllReferencedScenes()
                .returns(["startup", "new-scene"]);

            const newScenes = updateProjectIndex(
                fakeDocument,
                true,
                false,
                fakeIndex,
