...
import { Index, type FlowControlEvent } from "../../../server/src/common/index";
    ...
    describe("Index", () => {
        (describe("Case-Insensitive Tokens", () => {
            it("should store global variables with case insensitivity", () => {
                const creationLocation = Location.create(
                    documentUri,
                ...
                expect(Array.from(globalVars.entries())).to.eql([
                    ["variable", creationLocation],
                ]);
            });
            ...
            it("should allow access to global variables' original capitalization", () => {
                ...
                const results = Array.from(
                    globalVars.caseInsensitiveKeysToKeys().entries(),
                );
            ...
            it("should store local variables with case insensitivity", () => {
                const creationLocation1 = Location.create(
                    documentUri,
                    Range.create(1, 0, 1, 5),
                );
                const creationLocation2 = Location.create(
                    documentUri,
                ...
                expect(Array.from(localVars.entries())).to.eql([
                    ["variable", [creationLocation1, creationLocation2]],
                ]);
            });
            ...
            it("should allow access to local variables' original capitalization, with the first capitalization winning", () => {
                ...
                const localVars = index.getLocalVariables(documentUri);
                const results = Array.from(
                    localVars.caseInsensitiveKeysToKeys().entries(),
                );
            ...
            it("should store subroutine local variables with case insensitivity", () => {
                ...
                const index = new Index();
                index.setSubroutineLocalVariables(
                    documentUri,
                    variableCreation,
                );

                const subroutineVars =
                    index.getSubroutineLocalVariables(documentUri);

                expect(Array.from(subroutineVars.entries())).to.eql([
                    ["variable", creationLocation],
                ]);
            });
            ...
            it("should store variable references with case insensitivity", () => {
                const referenceLocation1 = Location.create(
                    documentUri,
                    Range.create(1, 0, 1, 5),
                );
                const referenceLocation2 = Location.create(
                    documentUri,
                    Range.create(2, 0, 2, 5),
                );
                ...
                expect(references[0].range.start).to.eql({
                    line: 1,
                    character: 0,
                });
                expect(references[0].range.end).to.eql({
                    line: 1,
                    character: 5,
                });
                expect(references[1].range.start).to.eql({
                    line: 2,
                    character: 0,
                });
                expect(references[1].range.end).to.eql({
                    line: 2,
                    character: 5,
                });
            });
            ...
            const otherSceneEvents: FlowControlEvent[] = [
                {
                    ...
                    scene: "faker",
                    sceneLocation: Location.create(
                        otherSceneUri,
                        Range.create(8, 0, 8, 8),
                    ),
                },
            ];
            const index = new Index();
        ...
        it("should find achievement references in a scene", () => {
            const referenceLocation = Location.create(
                documentUri,
                Range.create(1, 1, 1, 7),
            );
            const achievementReferences = new Map([
                ["achieve", [referenceLocation]],
            ]);
            const index = new Index();
            index.setAchievementReferences(documentUri, achievementReferences);

            const references =
                index.getDocumentAchievementReferences(documentUri);
            ...
            expect(achieveReferences[0].range.start).to.eql({
                line: 1,
                character: 1,
            });
            expect(achieveReferences[0].range.end).to.eql({
                line: 1,
                character: 7,
            });
        });
        ...
        it("should find achievement references across all scenes", () => {
            const localReferenceLocation = Location.create(
                documentUri,
                Range.create(1, 1, 1, 7),
            );
            const localAchievementReferences = new Map([
                ["achieve", [localReferenceLocation]],
            ]);
            const otherReferenceLocation = Location.create(
                otherSceneUri,
                Range.create(9, 1, 9, 7),
            );
            const otherAchievementReferences = new Map([
                ["achieve", [otherReferenceLocation]],
            ]);
            const index = new Index();
            index.setAchievementReferences(
                documentUri,
                localAchievementReferences,
            );
            index.setAchievementReferences(
                otherSceneUri,
                otherAchievementReferences,
            );
        ...
        describe("Indexed Scenes", () => {
            ...
            it("should include document scope URIs in indexed scenes", () => {
                const index = new Index();
                index.setDocumentScopes(documentUri, {
