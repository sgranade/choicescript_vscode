...
function createMockIndex({
    ...
    variableReferences,
    flowControlEvents,
    flowControlEventsUri,
    scopes,
}: IndexArgs): SubstituteOf<ProjectIndex> {
    ...
    fakeIndex
        .getSubroutineLocalVariables(Arg.any())
        .returns(subroutineVariables);
    ...
    if (flowControlEventsUri === undefined) {
        ...
        fakeIndex
            .getFlowControlEvents(flowControlEventsUri)
            .returns(flowControlEvents);
    }
...
describe("Definitions", () => {
    describe("Variable Definitions", () => {
        it("should not give definitions for non-references", () => {
            ...
            let localVariablesIndex: CaseInsensitiveMap<string, Location[]> =
                new CaseInsensitiveMap([["local_var", [createLocation]]]);
            let localVariables = new CaseInsensitiveMap([
                [documentUri, localVariablesIndex],
            ]);
        ...
        it("should locate a definition from a local variable reference", () => {
            ...
            let referenceLocation = Location.create(
                documentUri,
            ...
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([["local_var", [referenceLocation]]]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
            ]);
            ...
            let definition = (findDefinitions(
                documentUri,
                position,
                fakeIndex,
            ) ?? [])[0];
            ...
            expect(definition.location.range.start).to.eql({
                line: 1,
                character: 0,
            });
            expect(definition.location.range.end).to.eql({
                line: 1,
                character: 5,
            });
        });
        ...
        it("should locate all definition from a local variable reference", () => {
            ...
            let otherCreateLocation = Location.create(
                documentUri,
                Range.create(3, 0, 3, 5),
            );
            let referenceLocation = Location.create(
                documentUri,
                Range.create(2, 0, 2, 5),
            ...
            let localVariablesIndex: CaseInsensitiveMap<string, Location[]> =
                new CaseInsensitiveMap([
                    ["local_var", [createLocation, otherCreateLocation]],
                ]);
            let localVariables = new CaseInsensitiveMap([
                [documentUri, localVariablesIndex],
            ]);
            ...
            let definitions =
                findDefinitions(documentUri, position, fakeIndex) ?? [];

            expect(definitions.length).to.equal(2);
            expect(definitions[0].type).to.equal(SymbolType.LocalVariable);
            expect(definitions[0].location.range.start).to.eql({
                line: 1,
                character: 0,
            });
            expect(definitions[0].location.range.end).to.eql({
                line: 1,
                character: 5,
            });
            expect(definitions[1].type).to.equal(SymbolType.LocalVariable);
            expect(definitions[1].location.range.start).to.eql({
                line: 3,
                character: 0,
            });
            expect(definitions[1].location.range.end).to.eql({
                line: 3,
                character: 5,
            });
        });
        ...
        it("should locate a definition from a global variable reference", () => {
            ...
            let globalVariables: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([["global_var", createLocation]]);
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([["global_var", [referenceLocation]]]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
            ]);
            let position = Position.create(2, 2);
        ...
        it("should locate a definition from a global variable definition", () => {
            ...
            let definition = (findDefinitions(globalUri, position, fakeIndex) ??
                [])[0];
        ...
        it("should locate a local variable definition from a local variable reference when a global variable has the same name", () => {
            let localCreateLocation = Location.create(
                documentUri,
                Range.create(1, 0, 1, 5),
            );
            let localReferenceLocation = Location.create(
                documentUri,
                Range.create(2, 0, 2, 5),
            );
            let globalCreateLocation = Location.create(
                globalUri,
                Range.create(3, 0, 3, 5),
            );
            let globalVariables: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([
                    ["shared_var_name", globalCreateLocation],
                ]);
            let localVariablesIndex = new CaseInsensitiveMap([
                ["shared_var_name", [localCreateLocation]],
            ]);
            let localVariables = new CaseInsensitiveMap([
                [documentUri, localVariablesIndex],
            ]);
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["shared_var_name", [localReferenceLocation]],
                ]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
            ]);
        ...
        it("should locate a definition from an achievement variable reference", () => {
            ...
            let achievementLocation = Location.create(
                documentUri,
                Range.create(5, 0, 5, 5),
            );
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["choice_achieved_codename", [referenceLocation]],
                ]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
            ]);
            let achievementIndex: CaseInsensitiveMap<
                string,
                [Location, number, string]
            > = new CaseInsensitiveMap([
                ["codename", [achievementLocation, 0, ""]],
            ]);
            let position = Position.create(2, 2);
            let fakeIndex = createMockIndex({
                variableReferences: variableReferences,
    ...
    describe("Achievement Definitions", () => {
        ...
        it("should locate an achievement from its reference", () => {
            ...
            let referencesIndex: IdentifierMultiIndex = new CaseInsensitiveMap([
                ["codename", referenceLocations],
            ]);
            let references = new CaseInsensitiveMap([
                [documentUri, referencesIndex],
            ]);
            let position = Position.create(7, 2);
            let fakeIndex = createMockIndex({
                achievements: achievementIndex,
                achievementReferences: references,
            });
    ...
    describe("Label Definitions", () => {
        it("should locate local labels", () => {
            ...
            let labelLocation = Location.create(
                documentUri,
                Range.create(5, 0, 5, 5),
            );
            let labelsIndex: LabelIndex = new CaseInsensitiveMap([
                [
                    "local_label",
                    { label: "local_label", location: labelLocation },
                ],
            ]);
        ...
        it("should locate labels in other scenes", () => {
            ...
            let labelsIndex: LabelIndex = new CaseInsensitiveMap([
                [
                    "other_label",
                    { label: "other_label", location: labelLocation },
                ],
            ]);
...
describe("Symbol References", () => {
    describe("Variable References", () => {
        it("should give all local variable references", () => {
            ...
            let references =
                findReferences(documentUri, position, fakeContext, fakeIndex) ??
                [];

            expect(references.length).to.equal(1);
            expect(references[0].location.range.start).to.eql({
                line: 2,
                character: 0,
            });
            expect(references[0].location.range.end).to.eql({
                line: 2,
                character: 5,
            });
        });
        ...
        it("should include the local variable definition location at the end of the array if requested", () => {
            ...
            expect(references[1].location.range.start).to.eql({
                line: 1,
                character: 0,
            });
            expect(references[1].location.range.end).to.eql({
                line: 1,
                character: 5,
            });
        });
        ...
        it("should include all local variable definition locations at the end of the array if requested", () => {
            ...
            expect(references[2].location.range.start).to.eql({
                line: 3,
                character: 0,
            });
            expect(references[2].location.range.end).to.eql({
                line: 3,
                character: 5,
            });
        });
        ...
        it("should give all variable references for global variables on a reference", () => {
            ...
            let globalReferenceLocation = Location.create(
                globalUri,
                Range.create(3, 0, 3, 5),
            );
            let globalVariablesIndex: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([["global_var", createLocation]]);
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["global_var", [localReferenceLocation]],
                ]);
            let globalVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["global_var", [globalReferenceLocation]],
                ]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
                [globalUri, globalVariableReferences],
            ]);
            let position = Position.create(2, 1);
            let fakeContext = Substitute.for<ReferenceContext>();
            fakeContext.includeDeclaration.returns!(false);
            let fakeIndex = createMockIndex({
                globalVariables: globalVariablesIndex,
                variableReferences: variableReferences,
            });
        ...
        it("should give all variable references for a global variable on its creation", () => {
            ...
            let references =
                findReferences(globalUri, position, fakeContext, fakeIndex) ??
                [];
        ...
        it("should only give local variable references when a local and global variable share the same name", () => {
            ...
            let globalVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["shared_var_name", [globalReferenceLocation]],
                ]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
                [globalUri, globalVariableReferences],
    ...
    describe("Label References", () => {
        it("should give all matching label references on a label definition", () => {
            ...
            let documentLabelsIndex: LabelIndex = new CaseInsensitiveMap([
                [
                    "local_label",
                    { label: "local_label", location: labelLocation },
                ],
            ...
            let references =
                findReferences(documentUri, position, fakeContext, index) ?? [];
        ...
        it("should give all matching label references on a label reference", () => {
            ...
            let definitionLocation = Location.create(
                documentUri,
                Range.create(5, 0, 5, 5),
            );
            let documentLabelsIndex: LabelIndex = new CaseInsensitiveMap([
                [
                    "local_label",
                    { label: "local_label", location: definitionLocation },
                ],
            ]);
            // The logic for finding label references is complex enough that I'll use an actual Index
            let index = new Index();
        ...
        it("should ignore labels with the same name in a different scene", () => {
            ...
            let otherSceneLabelLocation = Location.create(
                otherSceneUri,
                Range.create(7, 0, 7, 7),
            );
            let otherSceneLabelsIndex: LabelIndex = new CaseInsensitiveMap([
                [
                    "local_label",
                    { label: "local_label", location: otherSceneLabelLocation },
                ],
            ]);
            // The logic for finding label references is complex enough that I'll use an actual Index
            let index = new Index();
        ...
        it("should find labels in a different scene on a reference", () => {
            ...
            let otherSceneLabelsIndex: LabelIndex = new CaseInsensitiveMap([
                [
                    "other_label",
                    { label: "other_label", location: otherSceneLabelLocation },
                ],
            ]);
            // The logic for finding label references is complex enough that I'll use an actual Index
            let index = new Index();
            index.setGlobalVariables(
                "file:///c:/startup.txt",
                new CaseInsensitiveMap(),
            );
            index.setFlowControlEvents(documentUri, events);
    ...
    describe("Achievement References", () => {
        it("should find achievement references from the achievement's definition", () => {
            ...
            let achievementReferencesIndex: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["codename", achievementReferenceLocations],
                ]);
            let achievementReferences = new CaseInsensitiveMap([
                [documentUri, achievementReferencesIndex],
            ]);
            let position = Position.create(5, 2);
            let fakeIndex = createMockIndex({
                achievements: achievementIndex,
        ...
        it("should find achievement references in another scene from the achievement's definition", () => {
            ...
            let achievementReferenceLocations = [
                Location.create(otherSceneUri, Range.create(7, 0, 7, 5)),
            ];
            let achievementReferencesIndex: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["codename", achievementReferenceLocations],
            ...
            let achievementReferences = new CaseInsensitiveMap([
                [otherSceneUri, achievementReferencesIndex],
            ]);
            let position = Position.create(5, 2);
            let fakeIndex = createMockIndex({
                achievements: achievementIndex,
...
describe("Symbol Renames", () => {
    describe("Variable Renames", () => {
        it("should rename local variables at definitions", () => {
            let createLocalLocation = Location.create(
                documentUri,
                Range.create(1, 0, 1, 5),
            );
            let localVariablesIndex = new CaseInsensitiveMap([
                ["local_var", [createLocalLocation]],
            ]);
            let localVariables = new CaseInsensitiveMap([
                [documentUri, localVariablesIndex],
            ]);
        ...
        it("should rename local variables at references", () => {
            ...
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["local_var", [localReferenceLocation]],
                ]);
            let variableReferences = new CaseInsensitiveMap([
                [documentUri, localVariableReferences],
            ]);
        ...
        it("should rename global variables at references", () => {
            let createGlobalLocation = Location.create(
                globalUri,
                Range.create(2, 0, 2, 5),
            );
            let globalVariables: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([["global_var", createGlobalLocation]]);
            let localReferenceLocation = Location.create(
                documentUri,
                Range.create(3, 0, 3, 5),
            );
            ...
            expect(localChanges[0].range.start).to.eql({
                line: 3,
                character: 0,
            });
            expect(localChanges[0].range.end).to.eql({ line: 3, character: 5 });
            expect(globalChanges[0].range.start).to.eql({
                line: 2,
                character: 0,
            });
            expect(globalChanges[0].range.end).to.eql({
                line: 2,
                character: 5,
            });
        });
        ...
        it("should only rename local variables locally", () => {
            ...
            let localVariablesIndex = new CaseInsensitiveMap([
                ["shared_var_name", [createLocalLocation]],
            ]);
            let localVariables = new CaseInsensitiveMap([
                [documentUri, localVariablesIndex],
            ]);
            let globalVariables: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([
                    ["shared_var_name", createGlobalLocation],
                ]);
            let localVariableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["shared_var_name", [localReferenceLocation]],
            ...
            expect(localChanges[0].range.end).to.eql({ line: 3, character: 5 });
            expect(localChanges[1].range.start).to.eql({
                line: 1,
                character: 0,
            });
    ...
    describe("Label Renames", () => {
        it("should rename all matching label references on a label definition", () => {
            let referenceLocation1 = Location.create(
                documentUri,
                Range.create(2, 0, 2, 5),
            );
            let referenceLocation2 = Location.create(
                documentUri,
                Range.create(4, 0, 4, 5),
            );
            let events: FlowControlEvent[] = [
            ...
            expect(localChanges[2].range.start).to.eql({
                line: 5,
                character: 0,
            });
        ...
        it("should rename labels in a different scene on a reference", () => {
            ...
            expect(Object.keys(allChanges)).to.eql([
                documentUri,
                otherSceneUri,
            ]);
            expect(otherChanges.length).to.equal(1);
            expect(otherChanges[0].range.start).to.eql({
                line: 7,
                character: 0,
            });
    ...
    describe("Achievement Renames", () => {
        ...
        it("should properly rename an achievement and its references", () => {
            ...
            expect(localChanges[1].newText).to.eql(
                "choice_achieved_new_achievement",
            );
