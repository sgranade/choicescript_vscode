...
import {
    generateDiagnostics,
    ValidationSettings,
} from "../../../server/src/common/validator";
...
function createIndex({
    ...
    fakeIndex
        .getDocumentVariableReferences(Arg.all())
        .returns(variableReferences);
...
function createValidationSettings(
    useCoGStyleGuide: boolean = true,
    allowUnsafeScript: AllowUnsafeScriptOption = "allow",
): SubstituteOf<ValidationSettings> {
...
describe("Validator", () => {
    describe("Style Validation", () => {
        it("should flag ellipses", async () => {
            ...
            expect(diagnostics[0].code).to.equal(
    ...
    describe("Variable Reference Validation", () => {
        it("should flag missing variables", async () => {
            ...
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([["unknown", [location]]]);
        ...
        it("should not flag a local variable referenced before it's created if a global variable exists", async () => {
            ...
            let localVariables = new CaseInsensitiveMap([
                ["var", [localCreateLocation]],
            ]);
            let globalVariables: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([["var", globalCreateLocation]]);
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([["var", [referenceLocation]]]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
        ...
        it("should not flag a local variable created through a gosub", async () => {
            let gosubLocation = Location.create(
                fakeDocumentUri,
                Range.create(1, 0, 1, 5),
            );
            let referenceLocation = Location.create(
            ...
            let createLocation = Location.create(
                fakeDocumentUri,
                Range.create(21, 0, 21, 5),
            );
            let localVariables = new CaseInsensitiveMap([
                ["local_var", [createLocation]],
            ]);
            let subroutineVariables: CaseInsensitiveMap<string, Location> =
                new CaseInsensitiveMap([["local_var", gosubLocation]]);
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([["local_var", [referenceLocation]]]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                localVariables: localVariables,
                variableReferences: variableReferences,
                subroutineVariables: subroutineVariables,
            });
            let fakeSettings = createValidationSettings();
        ...
        it("should not flag built-in variables", async () => {
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["choice_randomtest", [Substitute.for<Location>()]],
                ]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                variableReferences: variableReferences,
        ...
        it("should not flag param count", async () => {
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["param_count", [Substitute.for<Location>()]],
                ]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                variableReferences: variableReferences,
        ...
        it("should flag a non-param-count variable whose name contains 'param_count'", async () => {
            ...
            expect(diagnostics[0].message).to.include(
                '"not_actually_param_count" not defined',
            );
            expect(diagnostics[1].message).to.include(
                '"param_counter" not defined',
            );
        });
        ...
        it("should not flag param variables", async () => {
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["param_2", [Substitute.for<Location>()]],
                ]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                variableReferences: variableReferences,
        ...
        it("should flag achievement variables if not instantiated", async () => {
            let achievements: CaseInsensitiveMap<
                string,
                [Location, number, string]
            > = new CaseInsensitiveMap([
                ["codename", [Substitute.for<Location>(), 0, ""]],
            ]);
            let referenceLocation = Location.create(
                fakeDocumentUri,
                Range.create(1, 0, 1, 5),
            ...
            expect(diagnostics[0].message).to.contain(
                'Variable "choice_achieved_codename" not defined',
            );
        });
        ...
        it("should flag incorrect achievement variables", async () => {
            ...
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([
                    ["choice_achieved_othername", [referenceLocation]],
                ]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                variableReferences: variableReferences,
            ...
            expect(diagnostics.length).to.equal(1);
            expect(diagnostics[0].message).to.contain(
                'Variable "choice_achieved_othername" not defined',
            );
        });
        ...
        it("should not flag params variables after instantiation", async () => {
            ...
            let variableReferences: IdentifierMultiIndex =
                new CaseInsensitiveMap([["param_1", [referenceLocation]]]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                variableReferences: variableReferences,
                scopes: scopes,
    ...
    describe("All Commands Validation", () => {
        it("should flag commands with text in front of them", async () => {
            let fakeDocument = createDocument(
                "Leading text *if This is illegal!",
            );
            let fakeIndex = createIndex({});
            let fakeSettings = createValidationSettings();
    ...
    describe("Variable Creation Commands Validation", () => {
        it("should flag local variables with the same name as global ones", async () => {
            ...
            let globalVariables = new CaseInsensitiveMap([
                ["global_var", createLocation],
            ]);
            let localVariables = new CaseInsensitiveMap([
                ["global_var", [Substitute.for<Location>()]],
            ]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                globalVariables: globalVariables,
            ...
            expect(diagnostics.length).to.equal(1);
            expect(diagnostics[0].message).to.contain(
                "This variable has the same name as a global variable",
            );
        });
        ...
        it("should flag local variables with a repeated name", async () => {
            let localVariables = new CaseInsensitiveMap([
                [
                    "local_var",
                    [Substitute.for<Location>(), Substitute.for<Location>()],
                ],
            ]);
            let fakeDocument = createDocument("placeholder");
            ...
            expect(diagnostics.length).to.equal(1);
            expect(diagnostics[0].message).to.contain(
                "This variable was already defined earlier",
            );
        });

        it("should flag global variables that don't start with a letter", async () => {
            let globalVariables = new CaseInsensitiveMap([
                ["_invalid_var", Substitute.for<Location>()],
            ]);
    ...
    describe("Label Reference Commands Validation", () => {
        ...
        it("should flag missing label locations", async () => {
            ...
            expect(diagnostics[0].range.start).to.eql({
                line: 2,
                character: 0,
            });
        ...
        it("should be good with local labels", async () => {
            ...
            let localLabels: Map<string, Label> = new Map([
                ["local_label", Substitute.for<Label>()],
            ]);
            let fakeDocument = createDocument("placeholder");
            let fakeIndex = createIndex({
                labels: localLabels,
                labelsUri: fakeDocumentUri,
        ...
        it("should be good with labels in another scene", async () => {
            let sceneLabels: Map<string, Label> = new Map([
                ["scene_label", Substitute.for<Label>()],
            ]);
            let referenceLocation = Location.create(
                fakeDocumentUri,
                Range.create(2, 0, 2, 5),
            ...
            let fakeIndex = createIndex({
                flowControlEvents: events,
                labels: sceneLabels,
                labelsUri: fakeSceneUri,
                sceneList: ["other-scene"],
                sceneFileUri: fakeSceneUri,
            });
            let fakeSettings = createValidationSettings();
    ...
    describe("Achievement Validation", () => {
        it("should flag achievements with a repeated title", async () => {
            ...
            expect(diagnostics.length).to.equal(1);
            expect(diagnostics[0].message).to.contain(
                "the same title was defined earlier",
            );
            expect(diagnostics[0].range.start).to.eql({
                line: 2,
                character: 0,
        ...
        it("should flag more than 100 achievements", async () => {
            let achievements = new Map(
                [...Array(103).keys()].map(
                    (v): [string, [Location, number, string]] => [
                        `a${v}`,
                        [
                            Location.create(
                                fakeDocumentUri,
                                Range.create(v, 0, v, 5),
                            ),
                            1,
                            v.toString(),
                        ],
            ...
            expect(diagnostics[0].range.end).to.eql({
                line: 100,
                character: 5,
            });
        });
        ...
        it("should flag more than 1000 points' worth of achievements", async () => {
            ...
            expect(diagnostics.length).to.equal(2);
            expect(diagnostics[0].message).to.contain(
                "Total achievement points must be 1,000 or fewer. (This makes it 1100",
            );
            expect(diagnostics[0].range.start).to.eql({
                line: 10,
                character: 0,
            ...
            expect(diagnostics[1].message).to.contain(
                "Total achievement points must be 1,000 or fewer. (This makes it 1200",
            );
            expect(diagnostics[1].range.start).to.eql({
                line: 11,
                character: 0,
            });
    ...
    describe("Script Validation", () => {
        ...
        it("should diagnose *script commands with a warning if allowUnsafeScript is 'warn'", async () => {
            ...
            expect(diagnostics[0].message).to.contain(
                "Running games that use *script is a security-risk.",
            );
        });
