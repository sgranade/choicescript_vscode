...
import {
    CustomContext,
    Configuration,
    LocalWorkspaceStorageKeys,
    RandomtestPutResultsInDocumentOptions,
    RandomtestSettingsSource,
...
export function initializeTestProvider(): void {
    ...
    if (getPreviousRandomtestSettings()) {
        vscode.commands.executeCommand(
            "setContext",
            CustomContext.PreviousRandomtestSettingsExist,
            true,
        );
    }
}
 ...
 */
function getPreviousRandomtestSettings(): randomtestSettings | undefined {
    const localWSStorage = getWorkspaceStorageService();
    if (localWSStorage) {
        return localWSStorage.getValue<randomtestSettings>(
            LocalWorkspaceStorageKeys.PreviousRandomtestSettings,
        );
    }
}
...
function updatePreviousRandomtestSettings(newSettings: randomtestSettings) {
    const localWSStorage = getWorkspaceStorageService();
    if (localWSStorage) {
        localWSStorage.setValue<randomtestSettings>(
            LocalWorkspaceStorageKeys.PreviousRandomtestSettings,
            newSettings,
        );
    }
    ...
    if (document.length > 300000) {
        ...
        if (filename === undefined) {
            vscode.window.showErrorMessage(
                `Could not save test log file from ${document.uri.path}`,
                "OK",
            );
            return;
        }
        if (vscode.workspace.workspaceFolders === undefined) {
            vscode.window.showErrorMessage(
                `Could not save test log file; no workspace open`,
                "OK",
            );
            return;
        } else {
            vscode.window.setStatusBarMessage(
                `Too much test output; saving results to ${filename}`,
                5000,
            );
                ...
                if (!error) {
                    ...
                    if (fileSize > FILE_SIZE_LIMIT) {
                        vscode.window.showInformationMessage(
                            `Test results saved to ${filename} but the file is too large to automatically open`,
                            "OK",
                        );
                    } else {
            ...
            stream.on("error", (err) => {
                error = true;
                vscode.window.showErrorMessage(
                    `Failed to write the test log file: ${err}`,
                    "OK",
                );
            });
        ...
        vscode.workspace
            ...
            .then((doc) =>
                vscode.window.showTextDocument(
                    doc,
                    (vscode.window.activeTextEditor?.viewColumn ?? 0) + 1,
                ),
            );
    }
}
 ...
 */
async function getRandomtestSettings(
    source: RandomtestSettingsSource,
): Promise<randomtestSettings> {
    ...
    let settings: randomtestSettings = {
        iterations: config.get(Configuration.RandomtestIterations) ?? 1000,
        seed: config.get(Configuration.RandomtestSeed) ?? 0,
        showText: config.get(Configuration.RandomtestShowFullText) ?? false,
        avoidUsedOptions:
            config.get(Configuration.RandomtestAvoidUsedOptions) ?? true,
        showChoices: config.get(Configuration.RandomtestShowChoices) ?? false,
        showCoverage:
            config.get(Configuration.RandomtestShowLineCoverageStatistics) ??
            false,
        putResultsInUniqueDocument:
            config.get(Configuration.RandomtestPutResultsInUniqueDocument) ??
            true,
    ...
    if (source == RandomtestSettingsSource.LastTestRun) {
        ...
        if (previousRandomtestSettings) {
            ...
            settings.putResultsInUniqueDocument =
                config.get(
                    Configuration.RandomtestPutResultsInUniqueDocument,
                ) ?? true;
        }
    } else if (source == RandomtestSettingsSource.Interactive) {
        const booleanGroups: vscode.QuickPickItem[] = ["yes", "no"].map(
            (label) => ({ label }),
        );
        ...
        async function pickIterations(
            input: MultiStepInput,
            state: Partial<State>,
        ) {
            state.settings!.iterations = parseInt(
        ...
        async function pickShowText(
            ...
            return (input: MultiStepInput) =>
                pickAvoidUsedOptions(input, state);
        }

        async function pickAvoidUsedOptions(
            input: MultiStepInput,
            state: Partial<State>,
        ) {
            const pick = await input.showQuickPick({
                ...
                items: booleanGroups,
                activeItem:
                    booleanGroups[state.settings!.avoidUsedOptions ? 0 : 1],
                shouldResume: shouldResume,
            });
        ...
        async function pickShowChoices(
            input: MultiStepInput,
            state: Partial<State>,
        ) {
            const pick = await input.showQuickPick({
        ...
        async function pickShowCoverage(
            input: MultiStepInput,
            state: Partial<State>,
        ) {
            const pick = await input.showQuickPick({
                ...
                totalSteps: 6,
                placeholder:
                    "After the test, show how many times each line was encountered",
                items: booleanGroups,
        ...
        async function validateInt(entry: string) {
            ...
            return n !== Infinity && String(n) === entry && n >= 0
                ? undefined
                : "Not an integer";
        }
    ...
    const putResultsInDocumentConfig = config.get(
        Configuration.RandomtestPutResultsInDocument,
    );
    settings.putResultsInDocument =
        putResultsInDocumentConfig ===
            RandomtestPutResultsInDocumentOptions.Always ||
        (putResultsInDocumentConfig ===
            RandomtestPutResultsInDocumentOptions.Fulltext &&
            settings.showText);
    return settings;
}
...
export async function runRandomtest(
    ...
    let output: LogDocument | vscode.OutputChannel = outputChannel;
    if (settings.putResultsInDocument) {
        const logDocument = provider.getLogDocument(
            generateLogUri("Randomtest", true),
        );
        if (logDocument !== undefined) output = logDocument;
    }
...
function runTest(
    ...
    runningProcess.stdout?.on("data", (data: Buffer) => {
        ...
        lastLine = lastLine.trim().split("\n").pop() as string; // Since I may get multiple lines at once
    });
    ...
    runningProcess.on("exit", async (code?: number, signal?: string) => {
        ...
        if (code !== undefined) {
            ...
            vscode.window.showInformationMessage(
                `${testName} received unexpected signal: ${signal}`,
                "OK",
            );
        }
