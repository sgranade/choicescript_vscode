    ...
    client.onRequest(
        ...
        async (args: { uri: URI; encoding?: string }) => {
            return new TextDecoder().decode(
                await vscode.workspace.fs.readFile(vscode.Uri.file(args.uri)),
            );
        },
    );
    client.onRequest(
        ...
        async (args: { pattern: string; rootPath?: URI }) => {
            const filePaths = (
                await vscode.workspace.findFiles(args.pattern)
            ).map((f) => f.path);
            return args.rootPath === undefined
                ? filePaths
                : filePaths.map((path) => path.replace(args.rootPath!, "")); // relative path
        },
    );
