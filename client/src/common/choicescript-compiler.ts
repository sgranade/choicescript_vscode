...
export type AllScenesResult = {
    [key: string]: { labels: { [key: string]: number }; lines: string[] };
};
...
export class ChoiceScriptCompiler {
    constructor(private readonly workspaceProvider: IWorkspaceProvider) {}
        ...
        for (const s of scenes) {
            ...
            if (m) {
                ...
                const content = new TextDecoder().decode(
                    await this.workspaceProvider.fs.readFile(s),
                );
                allScenes[sceneName].lines = content.split(/\r?\n/);
                for (let l = 0; l < allScenes[sceneName].lines.length; l++) {
                    if (sceneName === "startup" && !title) {
                        const match =
                            allScenes[sceneName].lines[l].match(titleRegex);
                        if (match) {
                            title = match[1];
                        }
                    }
                    const labelMatch =
                        allScenes[sceneName].lines[l].match(labelRegex);
                    if (labelMatch) {
                        allScenes[sceneName].labels[
