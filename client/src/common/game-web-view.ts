...
import type {
    AllScenesResult,
    CompiledChoiceScriptGame,
} from "./choicescript-compiler";
...
export class GameWebViewManager {
    ...
    public async runCompiledGame(game: CompiledChoiceScriptGame) {
        ...
        if (this.panel) {
            this.panel.webview.html = (
                await this.getWebviewContent(game.scenes)
            ).toString();
        }
    }
    ...
    private async getWebviewContent(
        allScenes: AllScenesResult,
    ): Promise<string> {
        const view = this.panel?.webview;
        const allowEvalString =
            this.workspaceProvider.getConfiguration<AllowUnsafeScriptOption>(
        ...
        let content = new TextDecoder().decode(
            await vscode.workspace.fs.readFile(this.runIndexHtmlUri),
        );
                    ...
                    case "convert-resource-uri":
                        if (
                            ...
                            this.panel?.webview.postMessage({
                                command: "update-resource-uri",
                                nodeName: message.nodeName,
                                oldSrc: message.src,
                                newSrc: this.panel.webview
                                    .asWebviewUri(
                                        vscode.Uri.joinPath(
