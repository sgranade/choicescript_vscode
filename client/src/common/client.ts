...
} from "./interfaces/vscode-workspace-provider";
...
function annotateCSError(scene: string, line: number, message: string): void {
    ...
    vscode.workspace.openTextDocument(scenePath).then((document) => {
        vscode.window.showTextDocument(document).then((editor) => {
            ...
            annotationController.addTrailingAnnotation(
                editor,
                line,
                `Error: ${message.trim()}`,
            );
 ...
 */
function bbcodeDelimit(
    editor: vscode.TextEditor,
    delimitCharacters: string,
): void {
    if (!editor) {
        return;
    }
    ...
    if (selection.isEmpty) {
        editor
            .edit((edit) => edit.insert(selection.start, `${open}${close}`))
            .then(() => {
                const newStartIndex =
                    document.offsetAt(editor.selection.start) - close.length;
        ...
        editor
            .edit((edit) => edit.replace(selection, `${open}${word}${close}`))
            .then(() => {
                const newStartIndex =
                    document.offsetAt(editor.selection.start) + open.length;
                const newEndIndex =
                    document.offsetAt(editor.selection.end) - close.length;
                editor.selection = new vscode.Selection(
                    document.positionAt(newStartIndex),
...
function registerCommands(
    ...
    gameWebViewManager: GameWebViewManager,
    testProvider?: ChoiceScriptTestProvider,
) {
    const csCommands = [
        vscode.commands.registerTextEditorCommand(
            CustomCommands.Italicize,
            (editor) => {
    ...
    if (testProvider) {
        ...
        csCommands.push(
            vscode.commands.registerCommand(CustomCommands.RunQuicktest, () => {
                        ...
                        .then(() =>
                            testProvider.runQuicktest(
                                ...
                                context.asAbsolutePath(
                                    RelativePaths.Choicescript,
                                ),
                                sceneFilesPath,
                                imageFilesPath,
                                annotateCSError,
                                (running) =>
                                    controller.updateTestStatus(running),
                            ),
                        );
                }
            }),
            ...
            vscode.commands.registerCommand(CustomCommands.CancelTest, () => {
    ...
    csCommands.push(
        vscode.commands.registerCommand(CustomCommands.RunGame, async () => {
            const run = async () => {
                annotationController.clearAll();
                ...
                const compiledGame = await csCompiler.compile(
                    await workspaceProvider.findFiles(sceneFilesPath, "*.txt"),
                );
            ...
            if (gameWebViewManager.isRunning()) {
                const result = await vscode.window.showInformationMessage(
                    ...
                    {
                        detail: "Running again will destroy your current session.",
                        modal: true,
                    },
                    "Run",
                    "Focus",
...
function updateQuickSuggestions(): void {
    const quickSuggestionsState = !workspaceProvider.getConfiguration(
        Configuration.BaseSection,
        Configuration.DisableQuickSuggestions,
    );
...
export async function startClient(
    context: vscode.ExtensionContext,
    clientConstructor: LanguageClientConstructor,
    testProvider?: ChoiceScriptTestProvider,
    ...
    gameWebViewManager = new GameWebViewManager(
        context,
        annotateCSError,
        workspaceProvider,
    );
    ...
    const annotationsTextDocumentChangedSubscription =
        workspaceProvider.onDidChangeTextDocument(
            () => annotationController.onTextDocumentChanged,
    ...
    const configurationChangedSubscription =
        workspaceProvider.onDidChangeConfiguration(() => {
            updateQuickSuggestions();
            client.sendNotification(
                CustomMessages.CoGStyleGuide,
                workspaceProvider.getConfiguration(
                    Configuration.BaseSection,
                    Configuration.UseCOGStyleGuide,
                ),
            );
            client.sendNotification(
                CustomMessages.AllowUnsafeScript,
                workspaceProvider.getConfiguration<AllowUnsafeScriptOption>(
                    Configuration.BaseSection,
                    Configuration.AllowUnsafeScript,
                ),
            );
        });
