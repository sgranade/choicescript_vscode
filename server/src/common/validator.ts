...
const reuseCommandsLookup: ReadonlyMap<string, number> = new Map(
    reuseCommands.map((x) => [x, 1]),
);
...
function validateVariables(state: ValidationState): Diagnostic[] {
    ...
    for (const [variable, locations] of state.projectIndex
        .getLocalVariables(state.textDocumentUri)
        .entries()) {
        if (!startsWithLetterRegex.test(variable)) {
...
function validateLabelReference(
    ...
    if (
        ...
        (scene === undefined || scene[0] != "{")
    ) {
        const labelLocation = findLabelLocation(
            label,
            scene,
...
function validateReferences(state: ValidationState): Diagnostic[] {
    ...
    // Validate references
    const references = state.projectIndex.getDocumentVariableReferences(
        state.textDocumentUri,
    );
    ...
    for (const [variable, locations] of references.entries()) {
        ...
        const creationLocations = findVariableCreationLocations(
            variable,
            true,
            state.textDocumentUri,
            state.projectIndex,
        ...
        if (creationLocations !== undefined && creationLocations !== null) {
            ...
            const badLocations = locations.filter((location: Location) => {
                for (const creationLocation of creationLocations) {
                    if (
                        location.uri == creationLocation.uri &&
                        comparePositions(
                            location.range.end,
                            creationLocation.range.start,
                        ) >= 0
                    ) {
                        return false;
                    }
            ...
            if (badLocations.length > 0) {
                ...
                if (
                    ...
                    !state.projectIndex.getGlobalVariables().has(variable)
                ) {
                    const newDiagnostics = badLocations.map(
            ...
            !builtinVariables.test(variable) &&
            !paramValues.test(variable) &&
            projectIsIndexed
        ) {
            const scopes = state.projectIndex.getDocumentScopes(
                state.textDocumentUri,
            );
            ...
            if (
                scopes.achievementVarScopes.length > 0 &&
                variableIsAchievement(
                    variable,
                    state.projectIndex.getAchievements(),
                ) !== undefined
            ) {
                for (const scopeRange of scopes.achievementVarScopes) {
                    trimmedLocations = locations.filter(
                        (location: Location) => {
            ...
            const newDiagnostics = trimmedLocations.map(
                (location: Location): Diagnostic => {
                    return createDiagnosticFromLocation(
...
function validateFlowControlEvents(state: ValidationState): Diagnostic[] {
    ...
    for (const event of state.projectIndex.getFlowControlEvents(
        state.textDocumentUri,
    )) {
        if (event.scene != "" && event.sceneLocation !== undefined) {
            ...
            if (projectIsIndexed) {
                let diagnostic = validateSceneReference(
                    event.scene,
                    event.sceneLocation,
                    state,
                );
 ...
 * @returns Diagnostic message, if any.
 */
function validateCommandInLine(
    command: string,
    index: number,
    state: ValidationState,
): Diagnostic | undefined {
    ...
    if (validCommandsLookup.has(command)) {
        ...
        if (
            (command == "if" || command == "selectable_if") &&
            reuseCommandsLookup.has(actualCommand)
        ) {
            return;
        }
        ...
        if (
            reuseCommandsLookup.has(command) &&
            (actualCommand == "if" || actualCommand == "selectable_if")
        ) {
            return;
        }
    ...
    if (state.validationSettings.useCoGStyleGuide) {
        ...
        while ((e = mulitStartRegex.exec(remainingOption))) {
            ...
            const tokens = tokenizeMultireplace(
                remainingOption,
                state.textDocument,
                index + remainingLocalIndex + contentsSublocalIndex,
                contentsSublocalIndex,
            );
            ...
            if (pretextWordCount > 15 - runningWordCount) {
                const m = pretext.match(
                    `(\\S+?\\s+?){${15 - runningWordCount}}`,
                );
                if (m != null && m[0].length < pretext.length) {
                    overLimitLocalIndex =
                        remainingLocalIndex + (m.index ?? 0) + m[0].length;
                    break;
                }
            }
            ...
            if (tokens.body.length > 0) {
                ...
                if (longestWordCount > 15 - runningWordCount) {
                    const m = longestBody.text.match(
                        `(\\S+?\\s+?){${15 - runningWordCount}}`,
                    );
                    if (m != null && m[0].length < longestBody.text.length) {
                        overLimitLocalIndex =
                            remainingLocalIndex +
                            longestBody.localIndex +
                            (m.index ?? 0) +
                            m[0].length;
                        break;
                    }
            ...
            while (
                endIndex < remainingOption.length &&
                remainingOption[endIndex] != " " &&
                remainingOption[endIndex] != "\t"
            ) {
                endIndex++;
            }
        ...
        if (overLimitLocalIndex === undefined && remainingOption.trim() != "") {
            const m = remainingOption.match(
                `(\\S+?\\s+?){${15 - runningWordCount}}`,
            );
            if (
                m != null &&
                m[0].length + (m.index ?? 0) < remainingOption.length
            ) {
                overLimitLocalIndex =
                    remainingLocalIndex + (m.index ?? 0) + m[0].length;
            }
...
async function validateImages(
    state: ValidationState,
    fsProvider: FileSystemService,
): Promise<Diagnostic[]> {
    const diagnostics: Diagnostic[] = [];
    let imagePath = state.projectIndex.getPlatformImagePath();

    for (const [image, locations] of state.projectIndex.getImages(
        state.textDocumentUri,
    )) {
        let found: boolean;
        ...
        if (imagePath === undefined) {
            ...
            if (
                ...
                found = await fsProvider.fileExists(
                    path.join(imagePath, image),
                );
            }
            if (found) {
...
function validateScriptUsage(state: ValidationState): Diagnostic[] {
    ...
    for (const loc of state.projectIndex.getScriptUsages(
        state.textDocumentUri,
    )) {
...
const matchPattern = RegExp(
    `${stylePattern}|${incorrectCommandPattern}|${optionPattern}`,
    "g",
);
...
export async function generateDiagnostics(
    ...
    const diagnostics: Diagnostic[] = [
        ...projectIndex.getParseErrors(state.textDocumentUri),
    ];
    ...
    while ((m = matchPattern.exec(state.text))) {
        ...
        if (
            m.groups.styleGuide !== undefined &&
            state.validationSettings.useCoGStyleGuide
        ) {
            // Items against CoG styleguide
            const diagnostic = validateStyle(
                m.groups.styleGuide,
                m.index,
                state,
            );
            ...
            const diagnostic = validateCommandInLine(
                m.groups.command,
                m.index + m.groups.commandPrefix.length,
                state,
            );
            if (diagnostic !== undefined) diagnostics.push(diagnostic);
        } else if (m.groups.option !== undefined) {
            // The option match captures the entire line including the "#", so throw that away
            const diagnostic = validateOption(
                m.groups.option.slice(1),
                m.index + m[0].length - m.groups.option.length + 1,
                state,
            );
            if (diagnostic !== undefined) diagnostics.push(diagnostic);
        }
