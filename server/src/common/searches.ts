...
import type {
    ProjectIndex,
    ReadonlyIdentifierIndex,
    ReadonlyLabelIndex,
    ReadonlyIdentifierMultiIndex,
} from "./index";
import {
    variableIsAchievement,
    convertAchievementToVariable,
} from "./language";
...
export function findVariableCreationLocations(
    variable: string,
    considerEffectiveLocation: boolean,
    documentUri: string,
    index: ProjectIndex,
): Location[] | undefined {
 ...
 * @param index Project index.
 */
function findAchievementLocation(
    codename: string,
    index: ProjectIndex,
): Location | undefined {
 ...
 * @param index Index of symbols and their locations.
 */
function findMatchingMultiSymbol(
    documentUri: string,
    position: Position,
    index: ReadonlyIdentifierMultiIndex,
): SymbolLocation[] | undefined {
...
function findMatchingLabel(
    documentUri: string,
    position: Position,
    index: ReadonlyLabelIndex,
): SymbolLocation | undefined {
    for (const [symbol, label] of index.entries()) {
        if (
            label.location.uri == documentUri &&
            positionInRange(position, label.location.range)
        ) {
...
export function findDefinitions(
    ...
    if (localVariables !== undefined) {
        const symbolLocations = findMatchingMultiSymbol(
            documentUri,
            position,
            localVariables,
        );
    ...
    if (projectIndex.isStartupFileUri(documentUri)) {
        const symbolLocation = findMatchingSymbol(
            documentUri,
            position,
    ...
    for (const [variable, locations] of references.entries()) {
        ...
        if (match !== undefined) {
            const locations = findVariableCreationLocations(
                variable,
                false,
                documentUri,
                projectIndex,
    ...
    if (event !== undefined) {
        const location = findLabelLocation(
            event.label,
            event.scene,
            documentUri,
            projectIndex,
    ...
    const achievementReferences =
        projectIndex.getDocumentAchievementReferences(documentUri);
...
function findVariableReferences(
    ...
    if (definition.type == SymbolType.GlobalVariable) {
        information = projectIndex
            .getVariableReferences(definition.symbol)
            .map((reference) => {
                return {
        ...
        const localReferences = projectIndex.getDocumentVariableReferences(
            definition.location.uri,
        );
        const possibleLocations = localReferences.get(definition.symbol);
        if (possibleLocations !== undefined) {
            information = possibleLocations.map(
                (reference: Location): SymbolInformation => {
                    return {
                        symbol: definition.symbol,
                        location: reference,
                        type: definition.type,
 ...
 * @param projectIndex Project index.
 */
function findLabelReferences(
    definition: SymbolInformation,
    projectIndex: ProjectIndex,
): SymbolInformation[] {
    const information = projectIndex
        .getLabelReferences(definition.symbol)
        .map((reference) => {
            return {
                symbol: definition.symbol,
                location: reference,
 ...
 * @param projectIndex Project index.
 */
function findAchievementReferences(
    definition: SymbolInformation,
    projectIndex: ProjectIndex,
): SymbolInformation[] {
    const information = projectIndex
        .getAchievementReferences(definition.symbol)
        .map((reference) => {
            return {
                symbol: definition.symbol,
                location: reference,
    ...
    information.push(
        ...projectIndex
            .getVariableReferences(
                convertAchievementToVariable(definition.symbol),
            )
            .map((reference) => {
                return {
                    symbol: definition.symbol,
...
export function findReferences(
    ...
    if (
        firstDefinition.type == SymbolType.GlobalVariable ||
        firstDefinition.type == SymbolType.LocalVariable
    ) {
...
export function generateRenames(
    ...
    const referencesToChange = findReferences(
        textDocumentUri,
        position,
    ...
    for (const reference of referencesToChange) {
        ...
        if (
            definition.type == SymbolType.Achievement &&
            reference.type == SymbolType.LocalVariable
        ) {
