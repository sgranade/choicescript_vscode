...
import {
    CaseInsensitiveMap,
    type ReadonlyCaseInsensitiveMap,
    normalizeUri,
    mapToUnionedCaseInsensitiveMap,
} from "./utilities";
 ...
 */
export type ReadonlyIdentifierIndex = ReadonlyCaseInsensitiveMap<
    string,
    Location
>;
/**
 ...
 */
export type ReadonlyIdentifierMultiIndex = ReadonlyCaseInsensitiveMap<
    string,
    readonly Location[]
>;
/**
 ...
 */
export type ReadonlyLabelReferenceIndex = ReadonlyMap<
    string,
    readonly Location[]
>;
/**
 ...
 */
export type ReadonlyAchievementIndex = ReadonlyMap<
    string,
...
export interface ProjectIndex {
    ...
    setVariableReferences(
        sceneUri: string,
        newIndex: IdentifierMultiIndex | Map<string, Location[]>,
    ): void;
    /**
     ...
     * @param sceneUri Scene document URI.
     */
    getDocumentAchievementReferences(
        sceneUri: string,
...
export class Index implements ProjectIndex {
    ...
    setLocalVariables(
        ...
        this._localVariables.set(
            sceneUri,
            mapToUnionedCaseInsensitiveMap(newIndex),
        );
    }
    setSubroutineLocalVariables(
        sceneUri: string,
        ...
        this._subroutineLocalVariables.set(
            sceneUri,
            new CaseInsensitiveMap(newIndex),
        );
    }
    setVariableReferences(
        sceneUri: string,
    ...
    setAchievementReferences(
        ...
        this._achievementReferences.set(
            sceneUri,
            mapToUnionedCaseInsensitiveMap(newIndex),
        );
    }
    ...
    getLocalVariables(sceneUri: string): ReadonlyIdentifierMultiIndex {
        const index =
            this._localVariables.get(sceneUri) ?? new CaseInsensitiveMap();
        return index;
    }
    getSubroutineLocalVariables(sceneUri: string): ReadonlyIdentifierIndex {
        const index =
            this._subroutineLocalVariables.get(sceneUri) ??
    ...
    getDocumentAchievementReferences(
        ...
        const index =
            this._achievementReferences.get(sceneUri) ??
            new CaseInsensitiveMap();
        return index;
    }
    getAchievementReferences(achievement: string): readonly Location[] {
    ...
    getDocumentVariableReferences(
        ...
        const index =
            this._variableReferences.get(sceneUri) ?? new CaseInsensitiveMap();
        return index;
    }
    getVariableReferences(variable: string): readonly Location[] {
        const locations: Location[] = [];
        ...
        for (const events of this._flowControlEvents.values()) {
            scenes.push(
                ...events
                    .filter((event) => {
                        return (
                            event.command.endsWith("_scene") &&
                            event.scene != "" &&
                            !event.scene.startsWith("{")
                        );
                    })
    ...
    getLabelReferences(label: string): readonly Location[] {
        ...
        for (const [documentUri, events] of this._flowControlEvents) {
            const matchingEvents = events.filter((event) => {
                return (
                    event.labelLocation !== undefined && event.label == label
                );
            });
