 ...
 */
export function mapToUnionedCaseInsensitiveMap<K, V extends unknown[]>(
    map: Map<K, V>,
 ...
 */
export function caseInsensitiveMapToMap<K, V>(
    map: ReadonlyCaseInsensitiveMap<K, V>,
): Map<K, V> {
 ...
 */
export function readNextNonblankLine(
    text: string,
    lineStart: number,
): NewLine | undefined {
 ...
 */
export function extractToMatchingIndent(
    text: string,
    ...
    while (true) {
        ...
        if (
            nextLine === undefined ||
            (nextLine.splitLine?.padding.length ?? 0) <= initialIndent
        ) {
            break;
        }
...
export function rangeInOtherRange(range1: Range, range2: Range): boolean {
    return (
        comparePositions(range1.start, range2.start) >= 0 &&
