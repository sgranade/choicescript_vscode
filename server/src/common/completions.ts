...
import type {
    ...
    ReadonlyLabelIndex,
    LabelIndex,
    ReadonlyAchievementIndex,
} from "./index";
import {
    validCommandsCompletions,
    startupCommandsCompletions,
    uriIsStartupFile,
} from "./language";
import {
...
function generateCompletionsFromArray(
...
function generateCompletionsFromIndex(
    ...
    return Array.from(
        iteratorMap(index.keys(), (x: string) => ({
            label: x,
            kind: kind,
            data: dataDescription,
        })),
...
function generateVariableCompletions(
    ...
    for (const [variable, location] of projectIndex
        .getSubroutineLocalVariables(documentUri)
        .entries()) {
        const existingLocations = Array.from(
            localVariables.get(variable) ?? [],
        );
    ...
    const availableVariablesGenerator = iteratorFilter(
        finalLocalVariables.entries(),
        ([variable, locations]: [string, readonly Location[]]) => {
    ...
    const completions = Array.from(
        iteratorMap(
            availableVariablesGenerator,
            ([variable, location]: [string, readonly Location[]]) => ({
                // eslint-disable-line @typescript-eslint/no-unused-vars
                label: variable,
    ...
    for (const scope of projectIndex.getDocumentScopes(documentUri)
    ...
    if (includeAchievements) {
        completions.push(
            ...Array.from(
                iteratorMap(
                    projectIndex.getAchievements().keys(),
                    (x: string) => ({
        ...
        if (text[start] == "*") {
            ...
            if (tokens.length == 1) {
                ...
                switch (tokens[0]) {
                    ...
                    case "goto":
                        if (tokens.length == 2) {
                            completions = generateCompletionsFromIndex(
                                projectIndex.getLabels(documentUri),
                    ...
                    case "goto_scene":
                        if (tokens.length == 2) {
                            completions = generateCompletionsFromArray(
                                projectIndex.getIndexedScenes(),
                                CompletionItemKind.Reference,
                                "scenes",
                            );
                            ...
                            if (tokens[1].includes("-")) {
                                ...
                                completions.forEach((completion) => {
                                    completion.textEdit = TextEdit.replace(
                                        range,
                                        completion.label,
                                    );
                                });
                            ...
                            const sceneUri = projectIndex.getSceneUri(
                                tokens[1],
                            );
                            if (sceneUri !== undefined) {
                                completions = generateCompletionsFromIndex(
                                    projectIndex.getLabels(sceneUri),
                                    CompletionItemKind.Reference,
                                    "labels-scene",
                                );
                            }
        ...
        // Auto-complete variable references
            ...
            if (isMultireplace) {
                ...
                if (section == "}" || section == "\r" || section.trim() == "") {
                    ...
                    const balancedParens = extractToMatchingDelimiter(
                        innards,
                        "(",
                        ")",
                    );
