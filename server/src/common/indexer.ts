...
function generateScopes(state: IndexingState): DocumentScopes {
    ...
    if (state.checkAchievementLocation !== undefined) {
        ...
        scopes.achievementVarScopes.push(
            Range.create(start, documentEndLocation),
        );
    }
    for (const location of state.paramsLocations) {
        const start = location.range.start;
        scopes.paramScopes.push(Range.create(start, documentEndLocation));
    }
...
function* identifiersFirstLocationBetweenLocations(
    identifiers: ReadonlyMap<string, Location[]>,
    start: Position,
    end: Position,
...
function findSubroutineVariables(state: IndexingState): Map<string, Location> {
    ...
    const returnEvents = events.filter((event: FlowControlEvent) => {
        return event.command == "return";
    });
    ...
    for (const event of events) {
        ...
        if (
            comparePositions(
                labelLocation.location.range.end,
                event.commandLocation.range.start,
            ) < 0
        ) {
            continue;
        }
        ...
        const firstReturn = returnEvents.find((event: FlowControlEvent) => {
            return (
                event.commandLocation.range.start.line >
                labelLocation.location.range.start.line
            );
        });
        if (firstReturn === undefined) {
            continue;
        }
        for (const variable of identifiersFirstLocationBetweenLocations(
            localVariables,
            labelLocation.location.range.end,
            firstReturn.commandLocation.range.start,
        )) {
...
export function updateProjectIndex(
    ...
    isChoicescriptStatsFile: boolean,
    index: ProjectIndex,
): string[] {
    ...
    const callbacks: ParserCallbacks = {
        onCommand: (
            ...
            if (
                command == "check_achievements" &&
                indexingState.checkAchievementLocation === undefined
            ) {
        ...
        onVariableReference: (
            ...
            // eslint-disable-line @typescript-eslint/no-unused-vars
            const referenceArray: Location[] =
                indexingState.variableReferences.get(symbol) ?? [];
            ...
            if (command == "return") {
                ...
                if (size == 0) {
                    ...
                    label.scope = Range.create(
                        label.location.range.start,
                        commandLocation.range.end,
                    );
                }
            }
        },
        ...
        onAchievementCreate: (
            ...
            if (prevAchievement !== undefined) {
                ...
                indexingState.achievements.set(codename, [
                    location,
                    points,
                    title,
                ]);
        ...
        onAchievementReference: (
            ...
            // eslint-disable-line @typescript-eslint/no-unused-vars
            const referenceArray: Location[] =
                indexingState.achievementReferences.get(codename) ?? [];
            referenceArray.push(location);
        ...
        onImage: (symbol: string, location: Location, state: ParsingState) => {
            // eslint-disable-line @typescript-eslint/no-unused-vars
            const locations: Location[] =
                indexingState.achievementReferences.get(symbol) ?? [];
            locations.push(location);
    ...
    if (isStartupFile) {
        if (
            index.hasChoicescriptStats() &&
            !indexingState.scenes.includes("choicescript_stats")
        ) {
