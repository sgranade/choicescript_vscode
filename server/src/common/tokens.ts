...
const functionsLookup: ReadonlyMap<string, number> = new Map(
    functions.map((x) => [x, 1]),
);
const numberFunctionsLookup: ReadonlyMap<string, number> = new Map(
    numberFunctions.map((x) => [x, 1]),
);
const booleanFunctionsLookup: ReadonlyMap<string, number> = new Map(
    booleanFunctions.map((x) => [x, 1]),
);
const booleanNamedOperatorsLookup: ReadonlyMap<string, number> = new Map(
    booleanNamedOperators.map((x) => [x, 1]),
);
const numericNamedOperatorsLookup: ReadonlyMap<string, number> = new Map(
    numericNamedOperators.map((x) => [x, 1]),
);
const booleanNamedValuesLookup: ReadonlyMap<string, number> = new Map(
    booleanNamedValues.map((x) => [x, 1]),
);
const mathOperatorsLookup: ReadonlyMap<string, number> = new Map(
    mathOperators.map((x) => [x, 1]),
);
const comparisonOperatorsLookup: ReadonlyMap<string, number> = new Map(
    comparisonOperators.map((x) => [x, 1]),
);
const stringOperatorsLookup: ReadonlyMap<string, number> = new Map(
    stringOperators.map((x) => [x, 1]),
);
...
function isAnyOperator(token: ExpressionToken): boolean {
    return (
        token.type == ExpressionTokenType.MathOperator ||
 ...
 */
export function tokenEffectiveType(
    ...
    if (
        effectiveType == ExpressionTokenType.FunctionAndContents ||
    ...
    if (
        effectiveType == ExpressionTokenType.Parentheses &&
    ...
    if (operator.text !== "&") {
        switch (effectiveType) {
            case ExpressionTokenType.Number:
                if (
                    operator.type !== ExpressionTokenType.MathOperator &&
            ...
            case ExpressionTokenType.BooleanNamedValue:
                if (
                    operator.type !== ExpressionTokenType.BooleanNamedOperator
                ) {
            ...
            case ExpressionTokenType.String:
                if (operator.type == ExpressionTokenType.ComparisonOperator) {
                    ...
                    operator.type != ExpressionTokenType.StringOperator
                ) {
...
function determineEvalType(token: ExpressionToken): ExpressionEvalType {
    ...
    if (
        token.contents !== undefined &&
        token.contents.evalType == ExpressionEvalType.Error
    ) {
...
export class Expression {
     ...
     */
    private tokenizeUnprocessedToken(
        unprocessed: ExpressionToken,
    ): ExpressionToken[] {
     ...
     */
    private tokenizeRecursiveExpressions(
        expression: string,
    ): ExpressionToken[] {
        ...
        while ((m = recursivePatterns.exec(tokenizingExpression))) {
            ...
            if (m.groups.prefix !== undefined && m.groups.prefix != "") {
                partialTokens.push({
                    text: m.groups.prefix,
                    index: tokenizingIndex,
                    type: ExpressionTokenType.Unprocessed,
                });
            ...
            let contents = extractToMatchingDelimiter(
                m.groups.remainder,
                openDelimiter,
                closeDelimiter,
                0,
            ...
            partialTokens.push({
                text: contents,
                index: tokenizingIndex + openDelimiterIndex,
                type: tokenType,
                contents: tokenizedContents,
            });
            tokenizingExpression = tokenizingExpression.slice(
                openDelimiterIndex + contents.length,
            );
    ...
    private tokenizeExpression(expression: string): ExpressionToken[] {
        ...
        while ((m = arrayPattern.exec(expression))) {
            ...
            while (expression[localIndex] == "[") {
                // To deal with multi-dimensional arrays
                localIndex++;
                const reference = extractToMatchingDelimiter(
                    expression,
                    "[",
                    "]",
                    localIndex,
            ...
            expression =
                expression.slice(0, m.index) +
                " ".repeat(localIndex - m.index) +
                expression.slice(localIndex);
        }

        // Expressions can contain numbers, strings, operators, built-in variables, variables, and variable references
        // As variable references, strings, and parentheses can contain other things, tokenize them first
        const partialTokens: ExpressionToken[] =
            this.tokenizeRecursiveExpressions(expression);
    ...
    private validateExpression(): ExpressionEvalType {
        ...
        if (
            (this.combinedTokens[0].type == ExpressionTokenType.MathOperator ||
                this.combinedTokens[0].type ==
                    ExpressionTokenType.StringOperator) &&
            this.isValueSetting
        ) {
            const isNumberOperation =
                this.combinedTokens[0].type == ExpressionTokenType.MathOperator;
            ...
            if (
                isNumberOperation &&
                !isNumberCompatible(this.combinedTokens[1])
            ) {
            ...
            if (
                !isNumberOperation &&
                !isStringCompatible(this.combinedTokens[1])
            ) {
                this.validateErrors.push(
                    this.createTokenError(
                        this.combinedTokens[1],
            ...
            return isNumberOperation
                ? ExpressionEvalType.NumberChange
                : ExpressionEvalType.StringChange;
        }
        ...
        if (this.combinedTokens.length == 2) {
            if (isAnyOperator(this.combinedTokens[0])) {
                ...
                this.validateErrors.push(
                    this.createTokenError(this.combinedTokens[0], code),
                );
            } else {
                const index =
                    this.globalIndex + lastToken.index + lastToken.text.length;
        ...
        if (this.combinedTokens.length > 3) {
            ...
            if (
                this.combinedTokens[2].type ==
                    ExpressionTokenType.ComparisonOperator &&
                ...
                diagnostic = createDiagnostic(
                    ...
                    this.globalIndex +
                        this.combinedTokens[1].index +
                        this.combinedTokens[1].text.length,
                );
            } else if (
                this.combinedTokens[1].type ==
                    ExpressionTokenType.ComparisonOperator &&
                ...
                diagnostic = createDiagnostic(
                    ...
                    this.globalIndex + this.combinedTokens[2].index,
                    this.globalIndex +
                        this.combinedTokens[3].index +
                        this.combinedTokens[3].text.length,
                );
            } else {
                diagnostic = createDiagnostic(
                    DiagnosticCodes.TooManyExpressionElements,
    ...
    private validateSingleToken(token: ExpressionToken): boolean {
        ...
        if (token.type == ExpressionTokenType.UnknownOperator) {
            this.validateErrors.push(
                this.createTokenError(token, DiagnosticCodes.InvalidOperator),
            );
            return false;
        }
        ...
        if (
            (token.type == ExpressionTokenType.Parentheses ||
                token.type == ExpressionTokenType.FunctionAndContents) &&
            token.contents === undefined
        ) {
            this.validateErrors.push(
                this.createTokenError(
            ...
            this.validateErrors.push(
                this.createTokenError(token, DiagnosticCodes.MissingCloseBrace),
            );
            return false;
        }
        // ...except for strings
            ...
            this.validateErrors.push(
                this.createTokenError(token, DiagnosticCodes.MissingCloseQuote),
            );
            return false;
        }
        ...
        if (
            ...
            if (
                token.contents.evalType != ExpressionEvalType.Empty &&
            ...
            if (errorCode !== undefined) {
                const diagnostic = createDiagnostic(
                    ...
                    token.contents.globalIndex +
                        token.contents.bareExpression.length,
                );
                this.validateErrors.push(diagnostic);
                return false;
            }
     ...
     */
    private validateSingleTokenExpression(
        token: ExpressionToken,
    ): ExpressionEvalType {
        ...
        if (returnValue == ExpressionEvalType.Error || isAnyOperator(token)) {
            this.validateErrors.push(
                this.createTokenError(token, DiagnosticCodes.InvalidValue),
            );
            return ExpressionEvalType.Error;
        }
     ...
     */
    private validateThreeTokenExpression(
        first: ExpressionToken,
        operator: ExpressionToken,
        second: ExpressionToken,
    ): ExpressionEvalType {
        ...
        if (
            !this.validateSingleToken(first) ||
            !this.validateSingleToken(second)
        ) {
            return ExpressionEvalType.Error;
        }
        ...
        if (
            firstType == ExpressionTokenType.Variable ||
            ...
            if (diagnosticInfo !== undefined) {
                this.validateErrors.push(
                    this.createTokenError(second, ...diagnosticInfo),
                );
                return ExpressionEvalType.Error;
            }
        } else {
            ...
            if (diagnosticInfo !== undefined) {
                this.validateErrors.push(
                    this.createTokenError(operator, ...diagnosticInfo),
                );
                return ExpressionEvalType.Error;
            }
        ...
        if (
            (first.contents !== undefined &&
                first.contents.evalType == ExpressionEvalType.Error) ||
            (second.contents !== undefined &&
                second.contents.evalType == ExpressionEvalType.Error)
        ) {
            return ExpressionEvalType.Error;
        }
        ...
        if (
            operator.type == ExpressionTokenType.ComparisonOperator &&
            isAnyDefiniteValue(first)
        ) {
            if (isNumberCompatible(first) && !isNumberCompatible(second)) {
                neverTrue = true;
            } else if (
                isBooleanCompatible(first) &&
                !isBooleanCompatible(second)
            ) {
                neverTrue = true;
            } else if (
                isStringCompatible(first) &&
                !isStringCompatible(second)
            ) {
                neverTrue = true;
            }
        }
...
export function tokenizeMultireplace(
    section: string,
    textDocument: TextDocument,
    contentsGlobalIndex: number,
    contentsLocalIndex = 0,
    ...
    let workingText = extractToMatchingDelimiter(
        section,
        "{",
        "}",
        contentsLocalIndex,
    ...
    if (workingText === undefined) {
        ...
        if (fullLine === undefined) return undefined;
    ...
    if (workingText[0] == "(" || workingText[0] == "{") {
        ...
        if (testStartLocalIndex == workingText.length) {
            ...
            if (
                ...
                functionsLookup.has(
                    workingText.slice(testStartLocalIndex, testEndLocalIndex),
                )
            ) {
        ...
        test = new Expression(
            bareTest.text,
            testStartLocalIndex + contentsGlobalIndex,
            textDocument,
        );
    }
