...
if (fileExists(path.join(scenePath, "choicescript_stats.txt"))) {
...
Scene.prototype.verifySceneFile = function commandLineVerifySceneFile(sceneName) {
	...
	try {
		verifyFileName(scenePath, fileName);
		if (fullGame) {
			addFile(fileName);
		}
	} catch (e) {
	...
	(function () {
		...
		for (var i = 0; i < list.length; i++) {
			try {
				...
				verifyFileName(scenePath, fileName);
				var sceneText = slurpFile(path.join(scenePath, fileName), true /*throwOnError*/);
...
if (!exitCode) {
	...
	for (var sceneFile of fs.readdirSync(scenePath)) {
